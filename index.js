...
// Função getUser com Logs de Debug
// Função Inteligente para buscar Usuário (Resolve problema do 9º dígito)
...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        try {
            ...
            // ============================
            if (command === '!adv') {
                if (!isMaster) return; // Apenas Masters+
                
                // Formato: !adv global @user | motivo | tempo  OU  !adv @user | motivo
                const fullArgs = argText.split('|').map(a => a.trim());
                const isGlobal = args[0]?.toLowerCase() === 'global';
                
                let targetArg = isGlobal ? args[1] : args[0];
                let reason = fullArgs[1] || "Sem motivo especificado";
                let durationStr = isGlobal ? fullArgs[2] : null;

                const mentionedJid = msg.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
                let targetJid = mentionedJid ? mentionedJid : (targetArg ? targetArg.replace(/\D/g, '') + '@s.whatsapp.net' : null);

                if (!targetJid) return sock.sendMessage(jid, { text: '❌ Marque alguém ou digite o número.' });

                const targetUser = await getUser(targetJid);
                if (targetUser.isCanonized) return sock.sendMessage(jid, { text: '🛡️ Usuário Canonizado! Imune a advertências.' });
                const adminName = user.name;

                if (isGlobal) {
                    const id = getNextId(targetUser.globalWarnings, 'ADVG');
                    const endDate = parseDuration(durationStr) || moment().add(30, 'days').toDate();
                    
                    targetUser.globalWarnings.push({ id, reason, admin: adminName, duration: durationStr, endDate });
                    
                    // Checa Banimento Global (5 ADVs)
                    if (targetUser.globalWarnings.length >= 5) {
                        targetUser.embargo = { active: true, reason: "Acúmulo de 5 Advertências Globais", since: new Date(), admin: "SYSTEM" };
                        await sock.sendMessage(jid, { text: `⛔ **BANIMENTO GLOBAL APLICADO**\nO usuário @${cleanID(targetJid)} atingiu 5 advertências globais e entrou em EMBARGO automaticamente.`, mentions: [targetJid] });
                    } else {
                        const msgTxt = `📓| Caro(a) @${cleanID(targetJid)},\n\nVocê recebeu uma **ADVERTÊNCIA GLOBAL**.\n\n📄 Motivo: ${reason}\n⏳ Duração: ${durationStr}\n👮 Por: ${adminName}\n\n_Atenciosamente, Diretoria DEVS+_ 💜`;
                        await sock.sendMessage(targetJid, { text: msgTxt }); // SMS PV
                        await sock.sendMessage(jid, { text: `🟣 **ADV GLOBAL APLICADA**\nUsuário: @${cleanID(targetJid)}\nMotivo: ${reason}\nID: ${id}`, mentions: [targetJid] });
                    }
                } else {
                    // Local
                    ...
                    // Checa Banimento Local (3 ADVs no mesmo grupo)
                    const localCount = targetUser.localWarnings.filter(w => w.groupJid === jid).length;
                    
                    if (localCount >= 3) {
                         // Adiciona ao autoban do grupo
                         await GroupConfig.findOneAndUpdate({ jid }, { $push: { autoBanList: { jid: targetJid, reason: "Acúmulo de 3 Advertências Locais", admin: "SYSTEM" } } }, { upsert: true });
                         if(isSuperAdmin) await sock.groupParticipantsUpdate(jid, [targetJid], 'remove');
                         await sock.sendMessage(jid, { text: `🚫 **BANIMENTO LOCAL**\n@${cleanID(targetJid)} atingiu 3 advertências neste grupo e foi banido.`, mentions: [targetJid] });
                    } else {
                        await sock.sendMessage(jid, { text: `📕| @${cleanID(targetJid)} recebeu ${localCount}/3 advertências!\nRazão: ${reason}\nPor: ${adminName}`, mentions: [targetJid] });
                    }
                }
